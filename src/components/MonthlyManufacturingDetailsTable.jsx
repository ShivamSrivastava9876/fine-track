...
import { getDailyManufacturingReportDataAsync, getDailyReportList, getMonthlyManufacturingReportDataAsync, getMonthlyReportList, searchMonthlyManufacturingReportAsync } from "../redux/slice/report/reportSlice";
...
export default function MonthlyManufacturingDetailsTable() {
    ...
    const reportList = useSelector(getMonthlyReportList);
    ...
    const handleMonthlyManufacturingReportSearch = (e) => {
        e.preventDefault();
        dispatch(searchMonthlyManufacturingReportAsync(searchParameter)).then((result) => {
            if (searchMonthlyManufacturingReportAsync.fulfilled.match(result)) {
                // setSearchParameter("");
            }
        })
    }
    ...
    const downloadExcel = (data) => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "MonthlyManufacturingReport");
        XLSX.writeFile(workbook, "monthly_manufacturing_report.xlsx");
    }

    React.useEffect(() => {
        dispatch(getMonthlyManufacturingReportDataAsync())
    }, [dispatch])
    ...
    return (
        <div>
            <div className="flex items-center justify-between flex-wrap w-full mb-4">
                ...
                <h1 className="text-2xl mx-4 mb-4 font-bold">Monthly manufacturing report</h1>
                <div className="flex flex-wrap">
                    ...
                    <form onSubmit={(e) => handleMonthlyManufacturingReportSearch(e)} className="flex items-center m-2 md:w-80 border-2 border-solid border-gray-300 rounded-full px-4 py-2">
                        <input
                            type="search"
                            placeholder="Search"
                        ...
                        <div className="ml-2">
                            <Image
                                onClick={handleMonthlyManufacturingReportSearch}
                                src={SearchIcon}
                                alt="search-icon"
