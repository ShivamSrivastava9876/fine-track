...
const EditFormProduct = ({
  ...
  const [weightArray, setWeightArray] = useState(
    row.grossWeight && typeof row.grossWeight === "string"
      ? row.grossWeight.split(",")
      : []
  );
  ...
  const [stoneWeightArray, setStoneWeightArray] = useState(
    row.stoneWeight && typeof row.stoneWeight === "string"
      ? row.stoneWeight.split(",")
      : []
  );
  ...
  const [sizeArray, setSizeArray] = useState(
    row.size && typeof row.size === "string" ? row.size.split(",") : []
  );
  ...
  const [lengthArray, setLengthArray] = useState(
    row.length && typeof row.length === "string" ? row.length.split(",") : []
  );
      ...
      <div className="flex justify-center items-center fixed bottom-2 top-2 right-5 left-5 z-50">
        ...
        >
          <div className="my-2 bg-white">
            <h2 className="mx-2 my-3 text-2xl font-bold">प्रोडक्ट अपडेट करा</h2>
          </div>
          <div className="grid md:grid-cols-2 grid-cols-1 gap-2">
            <div class="relative cursor-pointer inline-block text-left mb-2 h-10 z-50">
              <div
                onClick={handleCategory}
                class="inline-flex items-center justify-center px-4 py-2 w-full rounded-xl border border-gray-300 shadow-sm bg-white text-sm font-medium text-gray-700 hover:text-gray-500 focus:outline-none focus:ring focus:ring-indigo-200 active:bg-gray-100 active:text-gray-600"
              >
            ...
            >
              <div class="inline-flex items-center justify-center px-4 py-2 w-full rounded-xl border border-gray-300 shadow-sm bg-white text-sm font-medium text-gray-700 hover:text-gray-500 focus:outline-none focus:ring focus:ring-indigo-200 active:bg-gray-100 active:text-gray-600">
                {productType || row.productType || "प्रोडक्टचा प्रकार"}
                {/* Arrow icon (tailwindcss/heroicons) */}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="w-5 h-5 ml-2 -mr-1 text-gray-400"
                  ...
                  {productTypeList.map((productType) => (
                    <div key={productType.id} class="py-1">
                      <div
                        href="#"
                        onClick={() =>
                          handleProductTypeClick(productType.product_type)
                        }
                        class="block px-4 py-2 text-sm cursor-pointer text-gray-700 hover:bg-indigo-100"
                      >
            ...
            <div className="mb-4 ">
              ...
              />
              <button
                onClick={(e) => addInputWeight(e)}
                className="p-2 rounded-full hover:bg-[#f8af77] text-white"
              >
              ...
              <div className="top-full left-0 right-0 md:w-21.375 overflow-x-auto p-2 bg-gray-100 rounded">
                {weightArray?.map((text, index) => (
                  <span
                    key={index}
                    className="inline-block bg-gray-200 p-2 rounded-md m-1"
                  >
                    {text}
                    <button
            ...
            {(productType.toLowerCase() === "necklace" ||
              productType.toLowerCase() === "ring") && (
              <div className="mb-4 ">
                <input
                  type="text"
                ...
                />
                <button
                  onClick={(e) => addInputStoneWeight(e)}
                  className="p-2 rounded-full hover:bg-[#f8af77] text-white"
                >
                  <Image src={AddIcon} alt="dashboard-icon" />
                </button>
            ...
            {(productType.toLowerCase() === "necklace" ||
              productType.toLowerCase() === "ring" ||
              productType.toLowerCase() === "bangle") && (
              <div className="mb-4 ">
                <input
                  type="text"
                  value={inputTextSize}
            ...
            {(productType.toLowerCase() === "bracelet" ||
              productType.toLowerCase() === "chain") && (
              <div className="mb-4 ">
                <input
                  type="text"
            ...
            <div className="mb-4 flex justify-center items-center">
              <label
                htmlFor="fileInput"
                className="w-full flex items-center cursor-pointer h-9 py-2 px-8 border rounded-xl outline-none border-[#9C9C9C] text-[#111010]"
              >
                <FiImage className="mr-2" />{" "}
                {files !== "" ? `${files} फोटो` : "फोटो टाका"}
                <input
                  type="file"
                  id="fileInput"
                  multiple
