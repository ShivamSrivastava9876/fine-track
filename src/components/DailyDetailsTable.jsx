...
import { getDailyReportDataAsync, getDailyReportList, searchDailyOrderReportAsync } from "../redux/slice/report/reportSlice";
...
export default function DailyDetailsTables() {
  ...
  const orderList = useSelector(getDailyReportList);
  ...
  const handleDailyOrderReportSearch = (e) => {
    e.preventDefault();
    dispatch(searchDailyOrderReportAsync(searchParameter)).then((result) => {
      if (searchDailyOrderReportAsync.fulfilled.match(result)) {
        // setSearchParameter("");
      }
    })
  }
  ...
  const downloadExcel = (data) => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "DailyOrderReport");
    XLSX.writeFile(workbook, "daily_order_report.xlsx");
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between flex-wrap w-full mb-4">
        <h1 className="text-2xl mx-4 mb-4 font-bold">Daily order report</h1>
        <div className="flex flex-wrap">
          ...
          <form onSubmit={(e) => handleDailyOrderReportSearch(e)} className="flex items-center m-2 md:w-80 border-2 border-solid border-gray-300 rounded-full px-4 py-2">
            <input
              type="search"
              placeholder="Search"
            ...
            <div className="ml-2">
              <Image
                onClick={handleDailyOrderReportSearch}
                src={SearchIcon}
                alt="search-icon"
