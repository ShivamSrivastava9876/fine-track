...
import { getManufacturingByWorkerData, getOrderByUserData, getOrderByUserReportAsync, getSelectedUserName, getSelectedWorkerName, getWorkerReportAsync, getWorkerReportList, getYearlyManufacturingReportDataAsync, getYearlyReportList, searchOrderByUserReportAsync } from "../redux/slice/report/reportSlice";
import { useRouter } from "next/router";

const columns = [
    // { id: "HuId", label: "HU ID", minWidth: 80 },
    { id: "productName", label: "प्रोडक्ट", minWidth: 150 },
...
export default function OrderByUserData() {
    ...
    const downloadExcel = (data) => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "OrderByUserReport");
        XLSX.writeFile(workbook, "order_by_user_report.xlsx");
    }

    const handleUserReportSearch = (e) => {
        e.preventDefault();
        dispatch(searchOrderByUserReportAsync({userId: router.query.userId, data: searchParameter})).then((result) => {
            if (searchOrderByUserReportAsync.fulfilled.match(result)) {
                // setSearchParameter("");
            }
        })
    }
    ...
    return (
        <div>
            <div className="flex items-center justify-between flex-wrap w-full mb-4">
                <h1 className="text-xl mx-4 mb-4 font-bold text-gray-500">
                    {selectedUserName.length !== 0 ? (
                        <span><span className="text-black">{selectedUserName}</span> चा ग्राहक अहवाल</span>
                    ) : (
                        <span>ग्राहक अहवाल</span>
                    )}
                </h1>
            ...
            <Paper sx={{ width: "100%", overflow: "hidden" }} className="w-full">
                ...
                {selectedUserName.length === 0 && (<div className="text-black font-semibold text-base mx-4 mt-2 flex justify-center">माहिती उपलब्ध नाही</div>)}
                <TablePagination
