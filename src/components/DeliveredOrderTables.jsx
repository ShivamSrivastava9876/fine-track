...
import { getDeliveredOrderAsync, getDeliveredOrderData, getOrderList, getOrderListAsync } from "@/redux/slice/order/orderSlice";
...
export default function DeliveredOrderTables() {
  const dispatch = useDispatch();
  const [page, setPage] = React.useState(0);
  const [rows, setRows] = React.useState([]);
  const [rowsPerPage, setRowsPerPage] = React.useState(10);

  const orderList = useSelector(getDeliveredOrderData);
  ...
  React.useEffect(() => {
    ...
    dispatch(getDeliveredOrderAsync())
  }, [dispatch])
